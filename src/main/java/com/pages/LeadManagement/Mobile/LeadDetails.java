	...
	public LeadPage LeadDetailsPage(HashMap<String, String> args) throws Exception {
		...
		try {
			...
			if (!args.get("locality1").equalsIgnoreCase("")) {
				...
				actualResult +=  Locality1_Label.getText() + " label present in Locality1|";
				locality2.type(args.get("locality2"));
				actualResult +=  Locality2_Label.getText() + " label present in Locality2|";
				Landmark.type(args.get("Landmark"));
				actualResult +=  Landmark_Label.getText() + " label present in Landmark|";
				...
				if (InvalidPinCode.isPresent()) {
					...
					if (!args.get("PinCode").equalsIgnoreCase("")) {
						actualResult +=  "Automated select City - " + City.getAttribute("value")
								+ " And State - " + State.getAttribute("value") + " based on pincode|";
						LOGGER.info("Automated select City - " + City.getAttribute("value")
						+ " And State - " + State.getAttribute("value") + " based on pincode");
						args.put("ActualResult", actualResult);
					}
