...
public class Filter_Page extends AbstractPage {
	...
	@FindBy(xpath = "//*[@text()='Custom Date Range']")
	...
	@FindBy(xpath = "//*[@text()='Meeting Date']")
	private ExtendedWebElement MeetingDate;

	@FindBy(xpath = "//*[@text()='Modified Date']")
	...
	public Search_Lead filter_Page(HashMap<String, String> args) throws Exception {
		...
		try {
			...
			if (args.get("Filter_Flag").contains("Yes")) {
				...
				asert.assertEquals(Filter.getText(), "Filter");
				asert.assertAll();
				Filter.click();
				CU.takeScreenshot(args, driver, "Lead Filter Page");

				actualResult = actualResult
						+ CU.assertEquals("Created Date", CreatedDate.getText(), args);
				actualResult = actualResult
						+ CU.assertEquals("Meeting Date", MeetingDate.getText(), args);
				actualResult = actualResult
						+ CU.assertEquals("Modified Date", ModifiedDate.getText(), args);
				...
				actualResult = actualResult
						+ "Filters option is available next to sort option. under filter options all display |";
				LOGGER.info("click on filters and all options are available");
				actualResult +=  "check Filter options |";
				...
				if (args.get("FilterType").equalsIgnoreCase("CreatedDate")) {
					...
					if (args.get("FilterOption1").equalsIgnoreCase("Today")) {
						...
						CU.takeScreenshot(args, driver, "select dates Page");
						actualResult = actualResult
								+ "select from date and To date from custome date Range filter options |";
					}
